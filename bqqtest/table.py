...
class QueryLogicTest:
    ...
    def build(self):
        diff = Query(
            "diff",
            """
SELECT "+" AS mark , * FROM (SELECT TO_JSON_STRING(ACTUAL), ROW_NUMBER() OVER() AS n FROM ACTUAL EXCEPT DISTINCT SELECT TO_JSON_STRING(EXPECTED), ROW_NUMBER() OVER() AS n FROM EXPECTED) UNION ALL
SELECT "-" AS mark , * FROM (SELECT TO_JSON_STRING(EXPECTED), ROW_NUMBER() OVER() AS n FROM EXPECTED EXCEPT DISTINCT SELECT TO_JSON_STRING(ACTUAL), ROW_NUMBER() OVER() AS n FROM ACTUAL) ORDER BY n ASC
""",
            [],
            {},
        )
