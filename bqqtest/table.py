...
class QueryLogicTest:
    ...
    def build(self):
        diff = Query(
            "diff",
            """
SELECT "+" AS mark , * FROM (SELECT TO_JSON_STRING(ACTUAL) FROM ACTUAL EXCEPT DISTINCT SELECT TO_JSON_STRING(EXPECTED) FROM EXPECTED) UNION ALL
SELECT "-" AS mark , * FROM (SELECT TO_JSON_STRING(EXPECTED) FROM EXPECTED EXCEPT DISTINCT SELECT TO_JSON_STRING(ACTUAL)FROM ACTUAL)
""",
            [],
            {},
        )
